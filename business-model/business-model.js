...
async function checkDefinitionExists() {
    ...
    el.welcomeSubtitle.textContent = 'Vous devez d\'abord completer une Definition de projet avant de passer au Business Model Canvas.';
...
async function loadProjects() {
    ...
    if (definitions.length === 1) {
        ...
        el.welcomeSend.textContent = 'Commencer le Business Model Canvas';
    } else {
...
function startInterview() {
    ...
    const contextMessage = 'Je souhaite faire le Business Model Canvas pour mon projet "' + state.projetNom + '". Voici la definition de projet validee :\n\n' + extractDefinitionSummary(state.definitionData);
    
    addMessage('Business Model Canvas pour : ' + state.projetNom, 'user');
...
async function sendToAPI(message) {
    ...
    try {
        const response = await fetch(CONFIG.apiUrl, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ mode: 'chat', message, history: state.history.slice(0, -1), docType: 'business_model' })
        });
        hideTypingIndicator();
        const data = await response.json();
        ...
        if (data.action === 'generate') {
            ...
            disableChatInput();
            await delay(500);
            addMessage('Je vais generer votre canvas Business Model Canvas...', 'ai', false);
            showTypingIndicator();
...
function showCadrageComplete() {
    ...
    el.chatInput.disabled = true;
    el.chatSend.disabled = true;
    el.chatInput.placeholder = 'Business Model Canvas termine - Consultez votre document';
    el.chatInput.style.opacity = '0.6';
    el.chatSend.style.opacity = '0.6';

    if (state.documentCache['business_model']) {
        showDocument('business_model', state.documentCache['business_model'], {
...
async function generateDocumentsInBackground() {
    ...
    try {
        ...
        if (data.success) {
            ...
            state.documentCache['business_model'] = data.document.replace(/PLACEHOLDER_BASE_URL/g, baseUrl);
        }
    } catch (error) {
